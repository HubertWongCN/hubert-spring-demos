package org.hubert.springboot.file.pdf.itextpdf.service.impl;
...
import org.hubert.springboot.file.pdf.itextpdf.entity.User;
import org.hubert.springboot.file.pdf.itextpdf.pdf.MyHeaderFooterPageEventHelper;
...
public class PdfServiceImpl implements PdfService {
    ...
    public Document generateItextPdfDocument(OutputStream os) throws Exception {
        ...
        pdfWriter.setPageEvent(new MyHeaderFooterPageEventHelper("SpringBoot集成文件 - 集成POI之Excel导入导出",
                "SpringBoot集成ItextPDF导出", "Hubert's Blog",
                "https://hubertwongcn.github.io/"));

        // open
        document.open();

        // add content - pdf meta information
        document.addAuthor("hubert");
        document.addCreationDate();
        document.addTitle("hubert-pdf-itextpdf");
        document.addKeywords("pdf-hubert-keyword");
        document.addCreator("hubert");

        // add content -  page content

        // Title
        document.add(createTitle("SpringBoot集成文件 - 集成POI之Excel导入导出"));

        // Chapter 1
        document.add(createChapterH1("1. 知识准备"));
        document.add(createChapterH2("1.1 什么是POI"));
        document.add(createParagraph("Apache POI 是创建和维护操作各种符合Office Open XML（OOXML）标准和微软的OLE " +
