package org.hubert.springboot.file.word.poi.service.impl;

import lombok.extern.slf4j.Slf4j;
import org.apache.poi.openxml4j.exceptions.InvalidFormatException;
import org.apache.poi.util.Units;
import org.apache.poi.xwpf.usermodel.*;
import org.hubert.springboot.file.word.poi.entity.User;
...
public class WordServiceImpl implements WordService {
    ...
    public XWPFDocument generateWordXWPFDocument() {
        ...
        // Title
        createTitle(doc, "SpringBoot集成文件 - 集成POI之Word导出");

        // Chapter 1
        createChapterH1(doc, "1. 知识准备");
        createChapterH2(doc, "1.1 什么是POI");
        createParagraph(doc, "Apache POI 是创建和维护操作各种符合Office Open XML（OOXML）标准和微软的OLE " +
                "2复合文档格式（OLE2）的Java API。用它可以使用Java读取和创建,修改MS Excel文件.而且," +
                "还可以使用Java读取和创建MS Word和MSPowerPoint文件。更多请参考[官方文档](https://poi.apache.org/index.html)");
        ...
        try {
            ...
            run.addPicture(stream, Document.PICTURE_TYPE_PNG, "Generated", Units.toEMU(256),
                    Units.toEMU(256));
    ...
    private List<User> getUserList() {
        ...
        for (int i = 0; i < 5; i++) {
            userList.add(User.builder()
                    .id(Long.parseLong(i + ""))
                    .userName("hubert" + i).email("hubert@hubert.org" + i)
                    .phoneNumber(121231231231L)
